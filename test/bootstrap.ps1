...
try {
    ...
    $scriptHost = Start-Process -FilePath wscript.exe -ArgumentList @('//B','//Nologo',('"' + $launcher + '"'),'Host','Local',$port,('"' + $resultPath + '"')) -WindowStyle Hidden -PassThru
    ...
    if ($status.app -ne 'Collabsprite' -or -not $status.localOnly) { throw 'Falscher Serverstatus.' }
